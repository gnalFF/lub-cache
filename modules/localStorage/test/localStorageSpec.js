    ...
    describe('localStorage', function () {
        ...
        describe("auto expiring", function () {
            it("should wrap the value in an object, having expiry key", inject(function (lubStorage) {
                ...
                expect(cacheObject.created).toBe(time);
            }));
        ...
        it("should call failover method when localStorage expired", inject(function (lubStorage, $window) {
            ...
            var spy = spyOn(failOverHandler, "failOver").andCallThrough();

            var cache = lubStorage("test", {
                failOver:failOverHandler.failOver
            });
        ...
        it("should use a failover method removing random 10% or at least 1", inject(function (lubStorageFailover, lubStorageKeysHelper) {
            var zipped = lubStorageFailover({

            });
            expect(lubStorageKeysHelper(zipped).length).toBe(0);
            zipped = lubStorageFailover({
                a:"b",
                b:"b",
                c:"d",
                e:"f"
            });
            expect(lubStorageKeysHelper(zipped).length).toBe(3);

            var big = {};
            for(var i = 0;i<30;i++){
                big["t"+i] = 1;
            }
            expect(lubStorageKeysHelper(big).length).toBe(30);
            zipped = lubStorageFailover(big);
            expect(lubStorageKeysHelper(zipped).length).toBe(27);
        }));
    });
